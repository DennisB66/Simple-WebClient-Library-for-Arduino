...
void setup()
{
  ...
  PRINT( F( "# -----------------------------")) LF;           // show header
	PRINT( F( "# -  WebClient Blink example  -")) LF;
...
//Ethernet.hostName( SERVER_NAME);                          // not supported (yet)
  ...
  client.connect( "192.168.1.69", 80);
  ...
  client.setMethod( HTTP_PUT);
  client.setPath( "/blink");
...
void loop()
{
  ...
  client.setArgs( "state", state ? "on" : "off");
  client.connect();
  client.request(); delay(1000);
  client.receive();
  client.stop();
  client.print();   delay(5000);
}
